...
rpad = (str, width, char=' ')->
  (str + (char for i in [0...width]).join '').slice 0, width-1
...
class Shift
  ...
  print: =>
    ...
    ret = "#{format start, 'MM-DD'}  ##{rpad @guard_id, 5}"
    for min in [0...60]
...
class Calendar
  ...
  print: ->
    ...
    ret.push shift.print() for shift in @shifts when shift.naps.length
    ret.join "\n"
  ...
  guards: =>
    _.uniq (shift.guard_id for shift in @shifts)
  ...
  minutes_slept_by_guard: =>
    ...
    for guard in @guards()
      ...
      for shift in @shifts when shift.naps.length and shift.guard_id is guard
        ...
        for min in [0...60]
          ret[guard][min] ||= 0
          ret[guard][min]++ if shift.asleepAt addMinutes midnight, min
    ret

  sleepiest_minute_by_guard: =>
    ret = {}
    for guard, mins of @minutes_slept_by_guard()
      max = Math.max Object.values(mins)...
      continue unless max > 0

      for k, v of mins when v is max
        ret[guard] ||= parseInt k
    ret

  least_reliable_guard: ->
    ret = {}
    for guard, mins of @minutes_slept_by_guard()
      ret[guard] = Math.max Object.values(mins)...
    max_slept = Math.max Object.values(ret)...
    for guard, min of ret when min is max_slept
      return guard

  sleepiest_guard_minute: =>
    lrg = @least_reliable_guard()
    smbg = @sleepiest_minute_by_guard()
    return [parseInt(lrg), smbg[lrg]]
...
# log cal.print()
time 'runtime'
[guard, minute] = cal.sleepiest_guard_minute()
log guard * minute
timeEnd 'runtime'
